...
// <--- 1. Оновлений інтерфейс пропсів
interface AddressManagerProps {
  userId: string;
  userPhone?: string; 
}

export default function AddressManager({ userId, userPhone }: AddressManagerProps) {
  ...
  const handleSave = async (data: AddressFormData) => {
    ...
    if (newAddress) {
      ...
      toast.error('Помилка при збереженні адреси');
    }
  };
  ...
  const handleSetDefault = async (id: number) => {
    ...
    if (success) {
      toast.success('Основну адресу змінено');
      loadAddresses();
    }
  };
      ...
      <AnimatePresence mode="wait">
        {isAdding ? (
          <AddressForm 
            ...
            defaultPhone={userPhone} // <--- 2. Передаємо телефон у форму
          />
        ) : (
          <motion.div 
            key="list"
            ...
            {loading ? (
              ...
              <div className="h-48 flex flex-col items-center justify-center bg-gray-50 dark:bg-neutral-800/50 rounded-xl border-2 border-dashed border-gray-300 dark:border-neutral-700">
                ...
                <p className="text-gray-500 dark:text-neutral-400 text-center">
                  Ви ще не додали жодної адреси.<br/>
